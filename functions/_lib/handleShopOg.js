...
export async function handleShopOg(context) {
  ...
  try {
    ...
    // Dynamic product-grid image generated by Supabase Edge Function (heavy PNG
    // work stays off Cloudflare so the lightweight Pages Function stays free-tier safe).
    // Falls back to shop logo → CTM logo automatically inside the edge function.
    const SUPABASE_URL = env.VITE_SUPABASE_URL
