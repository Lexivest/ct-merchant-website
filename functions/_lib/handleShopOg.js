...
export async function handleShopOg(context) {
  ...
  try {
    ...
    // Dynamic product-grid image generated by Supabase Edge Function.
    // We include the anon key as a query param so WhatsApp's bot (no auth headers)
    // can fetch the image without getting a 401.
    const SUPABASE_URL = env.VITE_SUPABASE_URL
    const SUPABASE_KEY = env.VITE_SUPABASE_ANON_KEY
    const image = `${SUPABASE_URL}/functions/v1/og-image?id=${encodeURIComponent(shopId)}&apikey=${SUPABASE_KEY}`
