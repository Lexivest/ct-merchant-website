...
import { FaArrowLeft, FaLocationDot, FaChevronRight, FaStoreSlash, FaTriangleExclamation } from "react-icons/fa6"
...
function Area() {
  ...
  useEffect(() => {
    async function fetchAreaShops() {
      if (!areaId) {
        setError("Area ID missing.")
        setLoading(false)
        return
      }
      ...
      try {
        ...
        const [{ data: areaData, error: areaError }, { data: shopsData, error: shopsError }] =
          await Promise.all([
            supabase.from("areas").select("name").eq("id", areaId).single(),
            supabase.from("shops").select("*").eq("area_id", areaId).order("name", { ascending: true }),
          ])
        ...
        setShops(shopsData || [])
        document.title = `${areaData?.name || "Area"} | CTMerchant`
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] text-[#0F1111]">
      <div className="sticky top-0 z-50 bg-[#131921] shadow-[0_4px_6px_rgba(0,0,0,0.1)]">
        <header className="mx-auto flex w-full max-w-[800px] items-center gap-4 px-4 py-3 text-white">
          ...
          <div className="flex-1 truncate text-[1.15rem] font-bold tracking-[0.5px]">
            {loading ? "Loading..." : `Merchants in ${areaName}`}
          </div>
        </header>
      ...
      <div className="mx-auto w-full max-w-[800px] flex-1 px-4 py-5">
        {loading ? (
          <div className="flex flex-col items-center justify-center px-5 py-10 text-center text-[#888C8C]">
            <div className="mb-4 h-11 w-11 animate-spin rounded-full border-4 border-pink-100 border-t-pink-600" />
            <span className="font-semibold">Loading Shops...</span>
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center rounded-lg border border-[#D5D9D9] bg-white px-5 py-16 text-center text-[#888C8C]">
            ...
            <span className="font-semibold text-[#0F1111]">{error}</span>
            <button
              type="button"
              onClick={() => window.location.reload()}
              className="mt-4 text-base font-bold text-pink-600"
            >
              Tap to Retry
            </button>
          </div>
