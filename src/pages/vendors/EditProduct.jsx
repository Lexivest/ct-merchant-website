...
export default function EditProduct() {
  ...
  const handleFileSelect = (e, slot) => {
    ...
    if (file.size > MAX_UPLOAD_SIZE) return alert("File is too large! Max 4MB.");
  ...
  const applyWhiteBorders = () => {
    ...
    img.onload = () => {
      ...
      if (cropperRef.current?.cropper) {
        cropperRef.current.cropper.replace(canvas.toDataURL("image/jpeg", 1.0));
      }
      setIsFitting(false);
    };
    img.src = tempImage;
  ...
  const applyCrop = () => {
    ...
    const croppedCanvas = cropper.getCroppedCanvas({
      width: 800, height: 800, fillColor: "#FFFFFF",
      imageSmoothingEnabled: true, imageSmoothingQuality: "high",
    });

    const finalCanvas = document.createElement("canvas");
    finalCanvas.width = 800; finalCanvas.height = 800;
    ...
    <div className="flex min-h-screen flex-col bg-[#F3F4F6] text-[#0F1111] pb-12">
      ...
      <main className={`mx-auto w-full max-w-[680px] p-5 transition-opacity ${deleting ? 'opacity-40 pointer-events-none' : 'opacity-100'}`}>
        ...
        <div className="mb-6 rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] p-4">
          ...
          <p className="text-[0.85rem] text-[#475569] leading-relaxed">Tap an image slot below to open the studio. We auto-compress your images (Max 4MB input) to perfectly balance data savings and high resolution.</p>
        </div>
