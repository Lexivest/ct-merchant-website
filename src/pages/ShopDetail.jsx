...
function ShopDetail() {
  ...
  const usedProducts = useMemo(
    () => (products || []).filter((product) => product.condition === "Fairly Used"),
    [products]
  )
  ...
  useEffect(() => {
    async function bootstrap() {
      ...
      try {
        ...
        const cacheKey = `ctm_shop_v33_${shopId}`
        const cacheStr = localStorage.getItem(cacheKey)

        if (cacheStr) {
          try {
            const cached = JSON.parse(cacheStr)
            hydrateFromData(cached, false)
          } catch {
            // ignore cache parse errors
          }
        }

        const fresh = await fetchFreshData(shopId, session?.user || null)

        localStorage.setItem(cacheKey, JSON.stringify(fresh))
        hydrateFromData(fresh, true)
      } catch (error) {
        setErrorMessage(
          error?.message || "This shop could not be found or has been removed."
        )
      } finally {
        setLoading(false)
      }
  ...
  }, [contentReady, currentShop])

  function hydrateFromData(data, overwriteLikeState = true) {
    setCurrentShop(data.shop || null)
    setProducts(data.products || [])
    setApprovedNews(data.approvedNews || [])
    setShopBanner(data.shopBanner || "")
    setLikeCount(data.likeCount || 0)

    if (overwriteLikeState) {
      setHasLiked(Boolean(data.hasLiked))
    }

    setContentReady(Boolean(data.shop))
  }

  async function fetchFreshData(targetShopId, signedInUser) {
    ...
    if (signedInUser?.id) {
      ...
      if (signedInUser.id !== shopData.owner_id) {
        supabase
          .from("shop_views")
          .insert({
            shop_id: targetShopId,
            viewer_id: signedInUser.id,
          })
          .then(() => {})
          .catch(() => {})
      }
  ...
  function renderSocialButtons() {
    ...
    if (currentShop.website_url) {
      const url = currentShop.website_url.startsWith("http")
        ? currentShop.website_url
        : `https://${currentShop.website_url}`
      ...
      links.push(
        <a
          ...
          rel="noreferrer"
          title="Visit Website"
          className="social-btn rounded-lg bg-indigo-600 text-white transition hover:-translate-y-0.5 hover:text-white"
        >
          <FaGlobe />
        </a>
      )
    ...
    if (currentShop.facebook_url) {
      const url = currentShop.facebook_url.startsWith("http")
        ? currentShop.facebook_url
        : `https://${currentShop.facebook_url}`
      ...
      links.push(
        <a
          ...
          rel="noreferrer"
          title="Facebook"
          className="social-btn rounded-lg bg-[#1877F2] text-white transition hover:-translate-y-0.5 hover:text-white"
        >
          <FaFacebookF />
        </a>
      )
    ...
    if (currentShop.instagram_url) {
      const url = currentShop.instagram_url.startsWith("http")
        ? currentShop.instagram_url
        : `https://${currentShop.instagram_url}`
      ...
      links.push(
        <a
          ...
          rel="noreferrer"
          title="Instagram"
          className="social-btn rounded-lg bg-[linear-gradient(45deg,#f09433_0%,#e6683c_25%,#dc2743_50%,#cc2366_75%,#bc1888_100%)] text-white transition hover:-translate-y-0.5 hover:text-white"
        >
          <FaInstagram />
        </a>
      )
    ...
    if (currentShop.twitter_url) {
      const url = currentShop.twitter_url.startsWith("http")
        ? currentShop.twitter_url
        : `https://${currentShop.twitter_url}`
      ...
      links.push(
        <a
          ...
          rel="noreferrer"
          title="X (Twitter)"
          className="social-btn rounded-lg bg-black text-white transition hover:-translate-y-0.5 hover:text-white"
        >
          <FaXTwitter />
        </a>
      )
    ...
    if (currentShop.tiktok_url) {
      const url = currentShop.tiktok_url.startsWith("http")
        ? currentShop.tiktok_url
        : `https://${currentShop.tiktok_url}`
  ...
  if (loading && !contentReady) {
    return (
      <div className="min-h-screen bg-[#E3E6E6]">
        <header className="sticky top-0 z-[100] flex flex-col bg-[#131921] text-white shadow">
          <div className="flex items-center justify-between px-4 py-3">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <span className="truncate text-[1.15rem] font-bold">
                Shop Details
              </span>
            </div>
          </div>
