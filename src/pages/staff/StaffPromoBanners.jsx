...
import { FEATURED_BANNER_BACKGROUNDS } from "../../lib/featuredBannerEngine"
...
export default function StaffPromoBanners() {
  ...
  const [selectedCityId, setSelectedCityId] = useState("") // Empty = Global
  ...
  // Sync title/subtitle if a shop is selected
  useEffect(() => {
    if (selectedShop) {
  ...
  async function handlePublish() {
    ...
    try {
      ...
      notify({
        ...
        message: "The banner is now live in the marketplace.",
      })
  ...
  return (
    <StaffPortalShell
      activeKey="promo-banners"
      title="Promo Banner Studio"
      description="Create text-based colorful promo widgets with city-specific targeting and shop links."
    >
      <SectionHeading
        eyebrow="Marketplace Feature"
        title="Promo Banner Engine"
        description="Design striking banners for special offers, announcements, or featured 'Hot Deals' linked to specific shops."
      />
        ...
        <div className="grid grid-cols-1 gap-8 xl:grid-cols-[450px_1fr]">
          ...
          <div className="space-y-6 rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <div>
                <label className="mb-2 block text-xs font-black uppercase tracking-wider text-slate-500">Target City (Optional)</label>
                <select 
              ...
              <div>
                <label className="mb-2 block text-xs font-black uppercase tracking-wider text-slate-500">Link to Shop (Optional)</label>
                <select 
              ...
              <div>
                ...
                <input 
                  ...
                  placeholder="e.g. Valid until Friday"
                  className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold outline-none focus:border-pink-500"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-2 block text-xs font-black uppercase tracking-wider text-slate-500">Button Text</label>
                  <input 
                    type="text"
                    value={cta}
                ...
                <div>
                  <label className="mb-2 block text-xs font-black uppercase tracking-wider text-slate-500">Sort Order</label>
                  <input 
                    type="number"
                    value={sortOrder}
              ...
              <div>
                <label className="mb-2 block text-xs font-black uppercase tracking-wider text-slate-500">External Link (Optional)</label>
                <input 
                  type="text"
                  value={externalLink}
              ...
              <div>
                <label className="mb-3 block text-xs font-black uppercase tracking-wider text-slate-500">Background Style</label>
                <div className="grid grid-cols-2 gap-2">
                  {FEATURED_BANNER_BACKGROUNDS.map((bg) => (
                    <button
                      key={bg.key}
                      onClick={() => setBackgroundKey(bg.key)}
                      className={`h-12 rounded-xl bg-gradient-to-r ${bg.bg} p-1 transition ${backgroundKey === bg.key ? 'ring-2 ring-pink-500 ring-offset-2' : 'opacity-60 hover:opacity-100'}`}
                      title={bg.label}
                    >
          ...
          <div className="space-y-8">
            <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                ...
                <FaWandMagicSparkles className="text-pink-600" />
              </div>
              <PromoBannerPreview 
                title={title || "Your Banner Title"}
                subtitle={subtitle || "Your subtitle appears here"}
                backgroundKey={backgroundKey}
                isHotDeal={Boolean(selectedShopId)}
              />
            </div>

            <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="mb-6 text-xl font-black text-slate-900">Active Banners</h3>
              <div className="space-y-4">
                {banners.map(banner => (
                    ...
                    <div className="mb-4 flex items-start justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className={`rounded-full px-2 py-0.5 text-[10px] font-black uppercase ${banner.status === 'published' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                            {banner.status}
                          </span>
                          <span className="text-[10px] font-bold text-slate-400">
                            {banner.cities?.name || 'Global'} • {formatDateTime(banner.created_at)}
                          </span>
                        </div>
                      ...
                      <div className="flex gap-2 opacity-0 transition group-hover:opacity-100">
                        <button 
                          onClick={() => updateStatus(banner, banner.status === 'published' ? 'paused' : 'published')}
                          className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-slate-600 shadow-sm hover:text-pink-600"
                        >
                          {banner.status === 'published' ? <FaPause /> : <FaPlay />}
                        </button>
                        <button 
                          onClick={() => handleDelete(banner)}
                          className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-rose-600 shadow-sm hover:bg-rose-50"
                        >
                          <FaTrashCan />
                        </button>
                      </div>
                ...
                {banners.length === 0 && (
                  <div className="py-12 text-center">
                    <FaImage className="mx-auto mb-3 text-4xl text-slate-200" />
                    <p className="font-bold text-slate-400">No promo banners found.</p>
                  </div>
                )}
              </div>
            </div>
