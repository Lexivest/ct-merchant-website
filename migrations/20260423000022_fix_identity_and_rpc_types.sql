-- 1. Fix missing identity on shops and products tables
-- This ensures INSERT operations work without manually providing IDs.

DO $$ 
BEGIN
    -- Fix shops ID if not already identity
    IF NOT EXISTS (
    ...
    -- Fix products ID if not already identity
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- 2. Drop and Recreate RPC with more robust types (Double Precision for coordinates)
-- Attempt dropping with both possible signatures to be safe
...
DROP FUNCTION IF EXISTS public.register_or_update_shop(text, text, text, text, text, bigint, bigint, text, text, double precision, double precision, text, text, text, text, text, text, text, text, text, text, text, text, text);
...
GRANT EXECUTE ON FUNCTION public.register_or_update_shop(text, text, text, text, text, bigint, bigint, text, text, double precision, double precision, text, text, text, text, text, text, text, text, text, text, text, text, text) TO authenticated;
