-- Fix payment-proof approval on Postgres by avoiding FOR UPDATE across a LEFT JOIN.
-- The shop row still gets locked for the transaction; the nullable city row is read separately.
