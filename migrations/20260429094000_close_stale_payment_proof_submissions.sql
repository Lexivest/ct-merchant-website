-- Close stale/direct merchant proof submissions after payment state has changed.
-- This keeps old payment pages from creating new pending proofs after a
-- verification payment is confirmed or a service subscription is already active.
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE opp.merchant_id = NEW.merchant_id
      AND opp.shop_id = NEW.shop_id
      AND opp.payment_kind = NEW.payment_kind
      AND opp.status = 'pending'
  ) THEN
    RAISE EXCEPTION 'A payment receipt is already awaiting staff review.';
  END IF;
  ...
  IF NEW.payment_kind = 'physical_verification' THEN
    ...
    IF coalesce(v_shop.is_verified, false) OR v_shop.kyc_status = 'approved' THEN
      RAISE EXCEPTION 'This shop has already completed physical verification.';
    END IF;
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION 'Verification payment is already confirmed for this merchant.';
    END IF;
  ...
  ELSE
    ...
    IF v_shop.subscription_end_date IS NOT NULL AND v_shop.subscription_end_date > now() THEN
      RAISE EXCEPTION 'This shop already has an active subscription.';
    END IF;
...
CREATE UNIQUE INDEX IF NOT EXISTS offline_payment_proofs_one_pending_service_shop_uidx
ON public.offline_payment_proofs (merchant_id, shop_id, payment_kind)
WHERE status = 'pending' AND payment_kind = 'service_fee';
