-- Restore the SECURITY INVOKER / private-implementation split for
-- public.review_product_submission that was accidentally overwritten by
-- 20260520100000_remove_product_approval_notification.sql.
--
-- The public wrapper stays SECURITY INVOKER (satisfies Supabase linter).
-- The real work lives in private.review_product_submission (SECURITY DEFINER,
-- not exposed via /rest/v1/rpc/ because the private schema is not in
-- PostgREST's exposed schema list).
--
-- This version retains the 20260520 change: approvals no longer push a
-- notification (status is visible in the merchant dashboard); rejections
-- still fire a notification so vendors know to take action.

-- 1. Update the private implementation (no approval notification).
...
BEGIN
  ...
  IF v_action = 'approve' THEN
    -- Approval is silent: status becomes visible in the merchant dashboard.
...
-- 2. Replace the public function with a SECURITY INVOKER wrapper.
--    This satisfies the Supabase linter (no SECURITY DEFINER in the exposed
--    API schema) while keeping the same /rest/v1/rpc/ surface for the frontend.
