-- The previous migration (20260511120000) intended to fix the notification
-- action path for physical-verification payment rejections but set it to
-- /merchant-payment which is not a registered route in the SPA, causing a
-- 404 when merchants tapped the notification link.
--
-- The correct client-side route for the merchant payment page is /remita.
-- This migration restores that path.  Everything else in the function is
-- unchanged.
...
CREATE OR REPLACE FUNCTION public.process_offline_payment_review(
  ...
  p_new_end_date          timestamp with time zone    DEFAULT NULL,
