-- Let sponsored service offerings appear in the same market carousel as sponsored
-- products, while preserving the existing active-shop and active-service guards.
    ...
    'sponsored_products', (
      ...
      FROM (
        SELECT
          ...
          s.storefront_url AS shop_storefront_url,
          coalesce(s.is_service, false) AS shop_is_service,
          jsonb_build_object(
            'id', s.id,
            'name', s.name,
            'category', s.category,
            'is_service', coalesce(s.is_service, false),
            'image_url', s.image_url,
            'storefront_url', s.storefront_url
          ) AS shops
