-- Fix naming mismatch: drop and recreate to change parameter name

-- 1. Drop old function to allow parameter name change
DROP FUNCTION IF EXISTS public.register_or_update_shop(text, text, text, text, text, bigint, bigint, text, text, numeric, numeric, text, text, text, text, text, text, text, text, text, text, text, text, text);

-- 2. Recreate with fixed parameter name p_kyc_video_url
CREATE OR REPLACE FUNCTION public.register_or_update_shop(
    ...
    p_kyc_video_url text DEFAULT NULL, -- Fixed parameter name
...
-- 3. Update handle_shop_verification trigger function
...
BEGIN
    IF NEW.status = 'approved' AND OLD.status != 'approved' THEN
        NEW.kyc_video_url := NULL; -- Fixed column name
