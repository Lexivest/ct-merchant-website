-- Allow super admins to approve service subscription payments using the actual bank payment time.
-- Physical verification remains a current-time one-time confirmation.
...
CREATE OR REPLACE FUNCTION public.process_offline_payment_review(
  ...
  p_payment_effective_at timestamp with time zone DEFAULT NULL::timestamp with time zone
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_payment_effective_at timestamp with time zone;
  v_shop_name text;
  v_city_name text;
  v_plan_label text;
...
BEGIN
  ...
  IF p_action = 'approve' THEN
    ...
    ELSIF v_proof.payment_kind = 'service_fee' THEN
      ...
      v_payment_effective_at := coalesce(p_payment_effective_at, now());

      IF p_new_end_date IS NOT NULL THEN
        v_new_end_date := p_new_end_date;
      ELSE
        v_base_date := v_payment_effective_at;
      ...
      INSERT INTO public.service_fee_payments (
        ...
        v_proof.merchant_id, v_proof.shop_id, v_expected_amount, v_plan_key, v_final_ref, 'success', v_payment_effective_at
      );
    ELSE
    ...
    RETURN jsonb_build_object(
      ...
      'paymentEffectiveAt', v_payment_effective_at,
